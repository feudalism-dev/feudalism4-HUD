...
string contentItem;
string producedItem = "fertilizer";
list acceptedItems = ["stover", "scraps", "straw", "manure", "slop", "eggshells"];
integer maxContents = 1000;
...
turnCompost()
{
    debug("turn compost");
    llSetTimerEvent(0.0);
    debug("MakerFertilizer");
    compostQuantity = (integer)llLinksetDataRead("compost");
    currentQuantity = (integer)llLinksetDataRead("fertilizer");
    debug("compostQuantity before: " +(string)compostQuantity);
    debug("fertilizerQuantity before: " + (string)currentQuantity);
    if (compostQuantity > 20)
    {
//        debug("Convert compost to fertilizer");
        compostQuantity -= 20;
        currentQuantity += 10;
        if (currentQuantity > maxContents)
            currentQuantity = maxContents;
        if (compostQuantity < 0)
            compostQuantity = 0;  

    }
    else if (compostQuantity > 10)
    {
//        debug("Convert compost to fertilizer");
        compostQuantity -= 10;
        currentQuantity += 5;
    ...
    llLinksetDataWrite("compost", (string)compostQuantity);
    llLinksetDataWrite("fertilizer", (string)currentQuantity);
    debug("compostQuantity after: " +(string)compostQuantity);
    debug("fertilizerQuantity after: " + (string)currentQuantity);    
    updateMeter();  
    if (compostQuantity > 0)  
        llSetTimerEvent(compostTimer);     
}
...
updateMeter()
{
    ...
    level = llRound((compostQuantity * 10.0) / maxContents);
    integer level2 = llRound((currentQuantity * 10.0) / maxContents);
    debug("Level: " + (string)level);
    llSetText("Compost" +": [" + GetMeterString(level) + "]\nQuantity: " + (string)compostQuantity + "/" + (string)maxContents +"\n\n\nFertilizer" +": [" + GetMeterString(level2) + "]\nQuantity: " + (string)currentQuantity + "/" + (string)maxContents , <0,1,0>, 1.0);
}
...
{  
//llOwnerSay("addItem()"); 
    if (llListFindList(acceptedItems, [itemToAdd]) != -1)
    {
        debug("AddItem. quantityToAdd: " +(string)quantityToAdd);
        debug("AddItem. itemToAdd: " + itemToAdd);
        compostQuantity = (integer)llLinksetDataRead("compost");
        debug("Compost Quantity before adding: " + (string)compostQuantity);
        compostQuantity += quantityToAdd;
        if (compostQuantity > maxContents)
        {
            llRegionSayTo(ToucherID, 0, "You gave more items than could be stored as compost. Try turning the compost to convert compost to fertilizer or wait for it to happen naturally. Excess items dropped.");
            compostQuantity = maxContents;
        }
        else
        {
            llRegionSayTo(ToucherID, 0, "You gave " + (string)quantityToAdd + " of item: " + itemToAdd + " to container named: " + llGetObjectName() + " with container id of " + llGetObjectDesc() );
        }   
        llLinksetDataWrite("compost", (string)compostQuantity); 
        updateMeter();
    }
    ...
    {
        string text = "\nThis container only accepts the following items: \n";
        integer i;
        for (i = 0;  i < llGetListLength(acceptedItems); i++)
        {
            text += "🍲 " + llList2String(acceptedItems, i) + "\n";
        }
        text += "Please try again and only load items from this list.";
        llRegionSayTo(ToucherID, 0, text);
    }    
}
...
default
{
    ...
    {     
        ...
        {
            llOwnerSay( "Error: Description does not contain 2 fields. Field1: name for what this produces (ie fertilizer). Field 2: security mode (all, group or owner.");
            return; 
        }        
...
//llOwnerSay("Message from pouch received.");        
            ...
            if (action == "pouchworn")
            {
                if (fromKey == ToucherID)
                {
                   ...
                   // llRegionSayTo(ToucherID, 0, "This container only accepts " + llToLower(contentItem));     
                    gNames = ["Contents", "Add Item","Take Item", "TurnCompost"];
                    gMenuPosition = 0;
                    menuText = "\nThis container only accepts the following items: \n";
                    integer i;
                    for (i = 0;  i < llGetListLength(acceptedItems); i++)
                    {
                        menuText += llList2String(acceptedItems, i) + "\n";
                    }
            ...
            else
            {
                ...
                if (llGetListLength(params) == 2)
                {
                    itemToAdd = llList2String(params, 0);
...
//llOwnerSay("Take item");
                ...
                else if (action == "turncompost")
                {
                    debug("TurnCompost selected.");
                    turnCompost();   
                }
                else if (llListFindList(acceptedItems, [message]) != -1)            
                {
                    debug ("Picked one of the accepted items: " + message);  
                    pouchListener = llListen(itemChannel, "", "", "");                    
                    llSetTimerEvent(10.0);                     
                    llRegionSayTo(ToucherID, itemChannel, "check," + message);
                }
                else if (llListFindList(amountsButtons, [message]) != -1)            
                {
                    debug("Got amount to add: " + message);
                    quantityToAdd = (integer)message;
...
//llOwnerSay("Chest load item state, listen, channel: " + (string) channel + "name: " + name + ", id: " + (string) id + ", message " + message);  
        ...
        {      
            ...
            if (numberOfParms > 2)
                quantityToAdd = (integer)llList2String(messageParms, 2);  
            if (action == "cgiveitem")
            {
                // got a give message
                addItem();
...
//llOwnerSay("state entry takeItem"); 
        contentsQuantity = (integer)llLinksetDataRead(producedItem);
        contentsName = producedItem;   
