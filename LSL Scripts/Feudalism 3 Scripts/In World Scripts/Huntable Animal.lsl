...
//llOwnerSay("Attacker: " + (string)attacker); 
...
default
{
    ...
    {     
        ...
        llSay(0,"Setting up Feudalism RPG Huntable animal.");
        killedBy = "";
        if (llGetListLength(descriptionParms) == 4)
        {
            ...
            llSay(0,"Feudalism RPG Huntable animal ready.");
            displayText = animalName + ": health: " + (string)currentHealth + " CR: " + (string)meleeCR;
            llSetText(displayText, <1.0,1.0,1.0>, 1.0);
        ...
        llSay(0,"Error: the description must include 4 fields separated by commas: name of animal, health of animal, how long to wait to reset animal after kill, CR of hitting the animal with a melee weapon. In the format: deer,10,30,2");
...
//llOwnerSay("Pos: " + llList2String(attackParms,4));        
...
//llOwnerSay("Message received by dummy. " + act);        
