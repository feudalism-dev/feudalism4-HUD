...
init()
{
    ...
    if (llGetListLength(params) == 2)
    {
        ...
        successes = [];    
        CR = -1;    
        llSetText("Grain Silo restarting...",<1.0,1.0,1.0>,1.0);    
...
state invalidSetup
{
    state_entry()
    {
        ...
        llOwnerSay("The Grain Silo failed to setup properly. Please fix the config file and click to restart.");   
    }
...
state ready
{
    ...
    {            
        debug("State ready.");
        debug("The grain silo is ready for use.");
    ...
    touch_start(integer total_number)
    {
        ...
        { 
            ...
            {
                llRegionSayTo(ToucherID, 0, "You're too far away to use the grain silo."); 
            }          
        }  
        else
        {
...
state prepping
{
    state_entry()
    {
        ...
        llSetText(name + "grain silo being worked... checking for required items.",<1.0,1.0,1.0>,1.0);   
        llRegionSayTo(ToucherID, 0,"For every " + (string)llList2Integer(ingredientAmounts, 0) + " " + llList2String(ingredientNames, 0) + " you add to the Grain Silo, you will receive " + (string)itemGivePerClick + " " + recipeName);            
