...
const API = {
    ...
    /**
     * Get inventory page (Inventory v2 - pagination)
     * @param {string} characterId - The character document ID
    ...
    async getInventoryPage(characterId, page = 1, pageSize = 50) {
        if (!characterId) {
            console.error('[getInventoryPage] No characterId provided');
        ...
        try {
            console.log('[getInventoryPage] Querying subcollection for character:', characterId, 'page:', page, 'pageSize:', pageSize);
            
            // Use Firestore REST API for pagination
            const parentPath = `projects/feudalism4-rpg/databases/(default)/documents/characters/${characterId}`;
            const url = `https://firestore.googleapis.com/v1/projects/feudalism4-rpg/databases/(default)/documents:runQuery`;
            ...
            console.log('[getInventoryPage] Retrieved', items.length, 'items for page', page, 'of', totalPages);
    ...
    async checkItems(characterId, items) {
        ...
        try {
            // Get all inventory items using getInventoryPage
            const inventoryResult = await this.getInventoryPage(characterId, 1, 1000);
            const inventoryItems = inventoryResult.items || [];
            
            // Convert to map for quick lookup
            const inventoryMap = {};
            inventoryItems.forEach(item => {
                inventoryMap[item.id] = item.qty;
            });
            ...
            for (const item of items) {
                ...
                const available = inventoryMap[normalizedName] || 0;
