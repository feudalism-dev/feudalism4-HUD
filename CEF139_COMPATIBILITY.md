...
- Using `window.confirm` / `window.alert` / `window.prompt` — use `MoapDialogs.showConfirm()`, `MoapDialogs.showAlert()`, or `UI.showConfirmDialog()` / `UI.showAlertDialog()` (see `js/moap-dialogs.js`)
